...
+(BOOL) keyChainSaveData:(id)data withIdentifier:(NSString*)identifier{
    ...
    // 设置新的数据
    [saveQueryMutableDictionary setObject:[NSKeyedArchiver archivedDataWithRootObject:data]
...
+(id) keyChainReadData:(NSString*)identifier{
    ...
    [keyChainReadQueryMutableDictionary setObject:(id)kCFBooleanTrue
                                           forKey:(id)kSecReturnData];
    // TODO: 只返回搜索到的第一条数据
    [keyChainReadQueryMutableDictionary setObject:(id)kSecMatchLimitOne
