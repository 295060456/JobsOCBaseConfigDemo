...
* <font color=red>对系统的 `UITabBar` 通过**KVC**的方式替换为自定义的 `JobsTabBar`</font>
...
* 用导航控制器包裹每一个控制器。<font color=red>**使其每个控制器都具备`push`到其他控制器的功能**</font>
...
* 至少在`-(void)viewWillAppear:(BOOL)animated`以后的生命周期，实现：
